...
class OptionsUtil {
    ...
    _getKnownOptions(options, context) {
        const knownOptions = commonS8sx90is_node.ObjectValue.empty();
        ...
        for (const knownOptionKey in this.optionDefinitions) {
            if (this.optionDefinitions.hasOwnProperty(knownOptionKey)) {
                ...
                if (knownOptionKey in options) {
                    ...
                    if (optionDefinition.nestedOptions && commonS8sx90is_node.isPlainObject(optionValue)) {
                        ...
                        protoValue = commonS8sx90is_node.parseData(optionValue, context) ?? undefined;
                    }
                    if (protoValue) {
                        knownOptions.set(commonS8sx90is_node.FieldPath$1.fromServerFormat(optionDefinition.serverName), protoValue);
                    }
                }
            }
        }
    ...
    getOptionsProto(context, knownOptions, optionsOverride) {
        ...
        if (optionsOverride) {
            const optionsMap = new Map(commonS8sx90is_node.mapToArray(optionsOverride, (value, key) => [
                commonS8sx90is_node.FieldPath$1.fromServerFormat(key),
                value !== undefined ? commonS8sx90is_node.parseData(value, context) : null
            ]));
...
function isIMapValue(obj) {
    ...
    if ('fields' in obj && (obj.fields === null || commonS8sx90is_node.isPlainObject(obj.fields))) {
        return true;
    }
    return false;
}
...
function valueToDefaultExpr$1(value) {
    ...
    else if (commonS8sx90is_node.isPlainObject(value)) {
        result = _map(value);
    }
...
function vectorToExpr$1(value) {
    ...
    else if (value instanceof commonS8sx90is_node.VectorValue) {
        return constant(value);
    }
    else if (Array.isArray(value)) {
        return constant(commonS8sx90is_node.vector(value));
    }
    else {
...
function fieldOrExpression$1(value) {
    if (commonS8sx90is_node.isString(value)) {
...
class Expression {
    ...
    asBoolean() {
        if (this instanceof BooleanExpression) {
            ...
            throw new commonS8sx90is_node.FirestoreError('invalid-argument', `Conversion of type ${typeof this} to BooleanExpression not supported.`);
        }
    }
    subtract(subtrahend) {
...
function _field(nameOrPath, methodName) {
    if (typeof nameOrPath === 'string') {
        if (commonS8sx90is_node.DOCUMENT_KEY_NAME === nameOrPath) {
            return new Field(commonS8sx90is_node.documentId()._internalPath, methodName);
        }
        return new Field(commonS8sx90is_node.fieldPathFromArgument('field', nameOrPath), methodName);
    }
    else {
...
class Constant extends Expression {
     ...
     */
    _toProto(_) {
        commonS8sx90is_node.hardAssert(this._protoValue !== undefined, 0x00ed);
    ...
    _readUserData(context) {
        ...
        if (isFirestoreValue(this._protoValue)) {
            ...
            this._protoValue = commonS8sx90is_node.parseData(this.value, context);
        }
    }
}
...
class MapValue extends Expression {
    ...
    _toProto(serializer) {
        return commonS8sx90is_node.toMapValue(serializer, this.plainObject);
    }
}
/**
 *
...
function timestampTruncate(fieldNameOrExpression, granularity, timezone) {
    const internalGranularity = commonS8sx90is_node.isString(granularity)
...
class PipelineValueExpression extends Expression {
    ...
    _toProto(jsonProtoSerializer) {
        return commonS8sx90is_node.toPipelineValue(this.pipeline._toProto(jsonProtoSerializer));
    }
    /**
     * @internal
     */
...
class Ordering {
    ...
    _toProto(serializer) {
        return {
            mapValue: {
                fields: {
                    direction: commonS8sx90is_node.toStringValue(this.direction),
...
function isSelectable(val) {
    const candidate = val;
    return (candidate.selectable && commonS8sx90is_node.isString(candidate.alias) && isExpr(candidate.expr));
}
...
function isAliasedAggregate(val) {
    const candidate = val;
    return (commonS8sx90is_node.isString(candidate.alias) &&
...
class AddFields extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...
            args: [commonS8sx90is_node.toMapValue(serializer, this.fields)]
        };
    }
    _readUserData(context) {
...
class Define extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [commonS8sx90is_node.toMapValue(serializer, this.aliasedExpressions)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
class Aggregate extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [
                commonS8sx90is_node.toMapValue(serializer, this.accumulators),
                commonS8sx90is_node.toMapValue(serializer, this.groups)
            ]
        };
    }
    _readUserData(context) {
...
class Distinct extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [commonS8sx90is_node.toMapValue(serializer, this.groups)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
class DocumentsSource extends Stage {
    ...
    constructor(docPaths, options) {
        ...
        if (!docPaths || docPaths.length === 0) {
            throw new commonS8sx90is_node.FirestoreError(commonS8sx90is_node.Code.INVALID_ARGUMENT, 'Empty document paths are not allowed in DocumentsSource');
        }
        ...
        if (uniqueDocPaths.size !== paths.length) {
            throw new commonS8sx90is_node.FirestoreError(commonS8sx90is_node.Code.INVALID_ARGUMENT, 'Duplicate document paths are not allowed in DocumentsSource');
        }
    ...
    _toProto(serializer) {
        return {
            ...
            args: [
                ...
                commonS8sx90is_node.toStringValue(this.distanceMeasure)
            ]
        };
    }
    _readUserData(context) {
...
class Limit extends Stage {
    ...
    constructor(limit, options) {
        commonS8sx90is_node.hardAssert(!isNaN(limit) && limit !== Infinity && limit !== -Infinity, 0x882c);
        super(options);
        this.limit = limit;
    }
    /**
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [commonS8sx90is_node.toNumber(serializer, this.limit)]
        };
    }
}
class Offset extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [commonS8sx90is_node.toNumber(serializer, this.offset)]
        };
    }
}
class Select extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [commonS8sx90is_node.toMapValue(serializer, this.selections)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
class Sample extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [commonS8sx90is_node.toNumber(serializer, this.rate), commonS8sx90is_node.toStringValue(this.mode)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
class Union extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [commonS8sx90is_node.toPipelineValue(this.other._toProto(serializer))]
        };
    }
    _readUserData(context) {
...
class Replace extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [this.map._toProto(serializer), commonS8sx90is_node.toStringValue(Replace.MODE)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
function readUserDataHelper(expressionMap, context) {
    if (commonS8sx90is_node.isUserData(expressionMap)) {
...
function toPipelineBooleanExpr(f) {
    if (f instanceof commonS8sx90is_node.FieldFilter) {
        ...
        switch (f.op) {
            ...
            default:
                commonS8sx90is_node.fail(0x9047);
        }
    }
    else if (f instanceof commonS8sx90is_node.CompositeFilter) {
        switch (f.op) {
            ...
            default:
                commonS8sx90is_node.fail(0x89ea);
        }
    }
...
function toPipelineStages(query, db) {
    let pipeline;
    if (commonS8sx90is_node.isCollectionGroupQuery(query)) {
        pipeline = db.pipeline().collectionGroup(query.collectionGroup);
    }
    else if (commonS8sx90is_node.isDocumentQuery(query)) {
        pipeline = db.pipeline().documents([commonS8sx90is_node.doc(db, query.path.canonicalString())]);
    }
    else {
    ...
    // orders
    const orders = commonS8sx90is_node.queryNormalizedOrderBy(query);
...
function selectablesToObject(selectables) {
    ...
    for (const selectable of selectables) {
        ...
        if (typeof selectable === 'string') {
            ...
            commonS8sx90is_node.fail(0x5319, { selectable });
        }
        if (result[alias] !== undefined) {
            throw new commonS8sx90is_node.FirestoreError('invalid-argument', `Duplicate alias or field '${alias}'`);
        }
...
function aliasedAggregateToMap(aliasedAggregatees) {
    return aliasedAggregatees.reduce((map, selectable) => {
        if (map.get(selectable.alias) !== undefined) {
            throw new commonS8sx90is_node.FirestoreError('invalid-argument', `Duplicate alias or field '${selectable.alias}'`);
        }
...
function vectorToExpr(value) {
    if (value instanceof Expression) {
        ...
        const result = constant(commonS8sx90is_node.vector(value));
        return result;
    }
    else {
        throw new Error('Unsupported value: ' + typeof value);
...
class Pipeline {
    ...
    removeFields(fieldValueOrOptions, ...additionalFields) {
        // Process argument union(s) from method overloads
        const options = isField(fieldValueOrOptions) || commonS8sx90is_node.isString(fieldValueOrOptions)
            ? {}
            : fieldValueOrOptions;
        const fields = isField(fieldValueOrOptions) || commonS8sx90is_node.isString(fieldValueOrOptions)
            ? [fieldValueOrOptions, ...additionalFields]
            : fieldValueOrOptions.fields;
        // Convert user land convenience types to internal types
        const convertedFields = fields.map(f => commonS8sx90is_node.isString(f) ? field(f) : f);
        // Create stage object
    ...
    select(selectionOrOptions, ...additionalSelections) {
        // Process argument union(s) from method overloads
        const options = isSelectable(selectionOrOptions) || commonS8sx90is_node.isString(selectionOrOptions)
            ? {}
            : selectionOrOptions;
        const selections = isSelectable(selectionOrOptions) || commonS8sx90is_node.isString(selectionOrOptions)
    ...
    offset(offsetOrOptions) {
        ...
        let options;
        let offset;
        if (commonS8sx90is_node.isNumber(offsetOrOptions)) {
            options = {};
            offset = offsetOrOptions;
        }
        else {
    ...
    limit(limitOrOptions) {
        // Process argument union(s) from method overloads
        const options = commonS8sx90is_node.isNumber(limitOrOptions) ? {} : limitOrOptions;
        const limit = commonS8sx90is_node.isNumber(limitOrOptions)
            ? limitOrOptions
    ...
    distinct(groupOrOptions, ...additionalGroups) {
        // Process argument union(s) from method overloads
        const options = commonS8sx90is_node.isString(groupOrOptions) || isSelectable(groupOrOptions)
            ? {}
            : groupOrOptions;
        const groups = commonS8sx90is_node.isString(groupOrOptions) || isSelectable(groupOrOptions)
    ...
    replaceWith(valueOrOptions) {
        // Process argument union(s) from method overloads
        const options = commonS8sx90is_node.isString(valueOrOptions) || isExpr(valueOrOptions) ? {} : valueOrOptions;
        const fieldNameOrExpr = commonS8sx90is_node.isString(valueOrOptions) || isExpr(valueOrOptions)
            ? valueOrOptions
            : valueOrOptions.map;
        // Convert user land convenience types to internal types
    ...
    sample(documentsOrOptions) {
        // Process argument union(s) from method overloads
        const options = commonS8sx90is_node.isNumber(documentsOrOptions) ? {} : documentsOrOptions;
        let rate;
        let mode;
        if (commonS8sx90is_node.isNumber(documentsOrOptions)) {
            rate = documentsOrOptions;
            mode = 'documents';
        }
        else if (commonS8sx90is_node.isNumber(documentsOrOptions.documents)) {
    ...
    unnest(selectableOrOptions, indexField) {
        ...
        if (commonS8sx90is_node.isString(indexFieldName)) {
...
class PipelineSource {
    ...
    collection(collectionOrOptions) {
        // Process argument union(s) from method overloads
        const options = commonS8sx90is_node.isString(collectionOrOptions) ||
            commonS8sx90is_node.isCollectionReference(collectionOrOptions)
            ? {}
            : collectionOrOptions;
        const collectionRefOrString = commonS8sx90is_node.isString(collectionOrOptions) ||
            commonS8sx90is_node.isCollectionReference(collectionOrOptions)
            ? collectionOrOptions
            : collectionOrOptions.collection;
        // Validate that a user provided reference is for the same Firestore DB
        if (commonS8sx90is_node.isCollectionReference(collectionRefOrString)) {
            this._validateReference(collectionRefOrString);
        }
        // Convert user land convenience types to internal types
        const normalizedCollection = commonS8sx90is_node.isString(collectionRefOrString)
    ...
    collectionGroup(collectionIdOrOptions) {
        ...
        let collectionId;
        let options;
        if (commonS8sx90is_node.isString(collectionIdOrOptions)) {
    ...
    documents(docsOrOptions) {
        ...
        docs
            .filter(v => v instanceof commonS8sx90is_node.DocumentReference)
            .forEach(dr => this._validateReference(dr));
        // Convert user land convenience types to internal types
        const normalizedDocs = docs.map(doc => commonS8sx90is_node.isString(doc) ? doc : doc.path);
        // Create stage object
    ...
    _validateReference(reference) {
        ...
        if (!refDbId.isEqual(this.databaseId)) {
            throw new commonS8sx90is_node.FirestoreError(commonS8sx90is_node.Code.INVALID_ARGUMENT, `Invalid ${reference instanceof commonS8sx90is_node.CollectionReference
                ? 'CollectionReference'
                : 'DocumentReference'}. ` +
...
function subcollection(pathOrOptions) {
    ...
    let path;
    let options;
    if (commonS8sx90is_node.isString(pathOrOptions)) {
        path = pathOrOptions;
        options = {};
    }
    else {
...
class PipelineResult {
    ...
    get(fieldPath) {
        ...
        const value = this._fields.field(commonS8sx90is_node.fieldPathFromArgument('DocumentSnapshot.get', fieldPath));
        if (value !== null) {
...
function execute(pipeline) {
    if (!pipeline._db) {
        return Promise.reject(new commonS8sx90is_node.FirestoreError(commonS8sx90is_node.Code.FAILED_PRECONDITION, 'This pipeline was created without a database (e.g., as a subcollection pipeline) and cannot be executed directly. It can only be used as part of another pipeline.'));
    }
    const datastore = commonS8sx90is_node.getDatastore(pipeline._db);
    const firestore = commonS8sx90is_node.cast(pipeline._db, commonS8sx90is_node.Firestore);
    const userDataReader = commonS8sx90is_node.newUserDataReader(firestore);
    ...
    return commonS8sx90is_node.invokeExecutePipeline(datastore, structuredPipeline).then(result => {
        ...
        const docs = result
            ...
            ? new commonS8sx90is_node.DocumentReference(firestore, null, element.key)
 ...
 * ```
 */
commonS8sx90is_node.Firestore.prototype.pipeline = function () {
    const userDataWriter = new commonS8sx90is_node.LiteUserDataWriter(this);
    const userDataReader = commonS8sx90is_node.newUserDataReader(this);
