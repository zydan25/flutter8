...
export const sendWhatsAppOtp = onCall(
  { secrets: [WHATSAPP_SEND_URL, OTP_HASH_SECRET] },
  async (request) => {
    ...
    await ref.set({
      phone,
      hash: otpHash(phone, otp, OTP_HASH_SECRET.value()),
      sentAtMs: now,
...
export const verifyWhatsAppOtp = onCall(
  { secrets: [OTP_HASH_SECRET, ADMIN_PHONES] },
  async (request) => {
    const phone = normalizePhone(request.data?.phoneNumber);
    const otp = String(request.data?.otp || '').trim();
    if (!/^\d{8,15}$/.test(phone) || !/^\d{6}$/.test(otp)) {
      throw new HttpsError('invalid-argument', 'بيانات التحقق غير صالحة');
    }
    ...
    if (attempts >= 5) {
      await ref.delete();
      throw new HttpsError('resource-exhausted', 'تم تجاوز عدد المحاولات');
    }
...
export const createOrder = onCall(async (request) => {
  ...
  const snapshots = await Promise.all(
    items.map((item: any) => db.collection('products').doc(String(item.productId)).get()),
  );
