...
// Temporary compatibility layer: profile data is now owned by Flask.
...
export async function migrateLegacyUsersToFlask(): Promise<void> {
  const snapshot = await getDocs(collection(db, 'users'));
  if (!snapshot.empty) await mirrorCustomersToFlask(snapshot.docs.map((d) => toUser(d.data(), d.id)));
}

// Orders are now served by Flask; these names remain only for UI compatibility.
...
// One-time legacy Products bridge. Once migration is confirmed, these can be removed with Firebase itself.
export function subscribeToProducts(onChange: (products: Product[]) => void): Unsubscribe {
  return onSnapshot(collection(db, 'products'), (snapshot) => onChange(snapshot.docs.map((d) => ({ ...d.data(), id: d.id } as Product))), (error) => { console.error('Products migration read failed:', error); onChange([]); });
}
