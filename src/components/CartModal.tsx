...
import { convertBasePrice, formatCurrencyPrice, getDeliveryFee, getGovernorateMarkupMultiplier } from '../utils/pricing';
...
export const CartModal: React.FC<CartModalProps> = ({ isOpen, cartItems, user, currency, onClose, onUpdateQuantity, onRemoveItem, onClearCart, onOrderPlaced, onShowToast }) => {
  ...
  const subtotal = useMemo(() => {
    const multiplier = getGovernorateMarkupMultiplier(governorate);
    const base = cartItems.reduce((sum, item) => sum + Number(item.price || item.product?.price || 0) * Math.max(1, item.quantity || 1), 0);
    return Math.round(base * multiplier);
  }, [cartItems, governorate]);
  const shippingFee = useMemo(() => getDeliveryFee(governorate), [governorate]);
  const displayedShipping = currency === 'SAR' ? Math.round(shippingFee / getGovernorateMarkupMultiplier(governorate) / 535) : shippingFee;
  const total = subtotal + (cartItems.length ? displayedShipping : 0);
  ...
  const handleCheckout = async (event: React.FormEvent) => {
    ...
    try {
      const order = await createOrder({
        customerName: customerName.trim(), governorate, address: address.trim(), currency, paymentMethod,
      ...
      onClearCart();
      onClose();
      onShowToast(order.status === 'awaiting_payment' ? `تم إنشاء الطلب ${order.orderNumber}، افتح المحادثة لإتمام التحويل ✅` : `تم تأكيد الطلب رقم ${order.orderNumber} ✅`, 'success');
    } catch (error: any) {
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-black/60" onClick={onClose}>
      <div className="bg-white w-full max-w-md h-full shadow-2xl flex flex-col" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {!cartItems.length ? <div className="h-64 flex flex-col items-center justify-center text-center text-slate-500"><ShoppingBag className="w-10 h-10 text-purple-300 mb-2"/><h3 className="font-bold text-slate-800">السلة فارغة</h3></div> : <>
            {cartItems.map((item) => <div key={`${item.product.id}-${item.selectedColor}-${item.selectedSize}`} className="flex gap-3 bg-slate-50 p-3 rounded-2xl border border-slate-100"><img src={item.product.image} alt={item.product.name} className="w-16 h-16 rounded-xl object-cover"/><div className="flex-1 min-w-0"><h4 className="font-bold text-xs truncate">{item.product.name}</h4><div className="text-[10px] text-slate-500 mt-1">{item.selectedColor && `اللون: ${item.selectedColor} `}{item.selectedSize && `المقاس: ${item.selectedSize}`}</div><span className="block text-xs font-black text-purple-700 mt-1">{formatCurrencyPrice(Number(item.price || 0) * item.quantity, currency, governorate)}</span><div className="flex items-center gap-2 mt-2"><button onClick={() => onUpdateQuantity(item.product.id, Math.max(1, item.quantity - 1))} className="w-7 h-7 bg-white rounded-lg border">−</button><span className="text-xs font-bold">{item.quantity}</span><button onClick={() => onUpdateQuantity(item.product.id, item.quantity + 1)} className="w-7 h-7 bg-white rounded-lg border">+</button></div></div><button onClick={() => onRemoveItem(item.product.id)} className="text-slate-400 hover:text-rose-500 h-fit"><Trash2 className="w-4 h-4"/></button></div>)}
        ...
        {!!cartItems.length && <div className="p-4 border-t bg-slate-50 space-y-2"><div className="flex justify-between text-xs"><span>قيمة المنتجات</span><b>{formatCurrencyPrice(subtotal, currency, governorate)}</b></div><div className="flex justify-between text-xs"><span>التوصيل</span><b>{displayedShipping === 0 ? 'مجاني' : formatCurrencyPrice(displayedShipping, currency, governorate)}</b></div><div className="text-[10px] text-slate-400">معادلة المحافظة مضافة إلى السعر: {Math.round((getGovernorateMarkupMultiplier(governorate) - 1) * 100)}%</div><div className="flex justify-between text-sm font-black text-purple-700 border-t pt-2"><span>الإجمالي</span><span>{formatCurrencyPrice(total, currency, governorate)}</span></div><button type="button" onClick={() => handleCheckout({ preventDefault: () => undefined } as React.FormEvent)} disabled={isSubmitting} className="w-full py-3 rounded-xl bg-purple-600 hover:bg-purple-700 disabled:opacity-50 text-white font-bold flex items-center justify-center gap-2"><CheckCircle className="w-4 h-4"/>{isSubmitting ? 'جاري إنشاء الطلب...' : paymentMethod === 'cash_on_delivery' ? 'تأكيد الطلب الآن' : 'إنشاء الطلب والانتقال للدفع'}<ArrowRight className="w-4 h-4"/></button></div>}
      </div>
    </div>
  );
};