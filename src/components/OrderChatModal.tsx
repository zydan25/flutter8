...
import { X, Send, Paperclip, CheckCircle, Truck, PackageCheck, Image as ImageIcon } from 'lucide-react';
import type { ChatMessage, Order, User } from '../types';
import { getOrderChat, sendOrderChatMessageApi, uploadPaymentProofApi, updateFlaskOrderStatus } from '../api';
...
export const OrderChatModal: React.FC<Props> = ({ isOpen, order, user, onClose, onOrderChanged, onShowToast }) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const result = await getOrderChat(order.id);
        if (active) setMessages((result.messages || []) as ChatMessage[]);
      } catch (error) {
        console.error('Flask chat load failed:', error);
  ...
  const sendText = async () => {
    ...
    setBusy(true);
    try {
      const result = await sendOrderChatMessageApi(order.id, { type: 'text', text: value });
      if (result.message) setMessages((current) => [...current.filter((m) => m.id !== result.message.id), result.message as ChatMessage]);
      setText('');
    } catch (error) {
      console.error(error);
      onShowToast('تعذر إرسال الرسالة', 'error');
    } finally { setBusy(false); }
  };

  const uploadProof = async (file: File) => {
    if (!user || user.isAdmin || !file.type.startsWith('image/') || file.size > 5 * 1024 * 1024) {
      onShowToast('اختر صورة أقل من 5MB', 'error');
      return;
    }
    setBusy(true);
    try {
      const result = await uploadPaymentProofApi(order.id, file, 'إشعار تحويل من العميل');
      if (result.message) setMessages((current) => [...current.filter((m) => m.id !== result.message.id), result.message as ChatMessage]);
      if (result.order) onOrderChanged?.(result.order as Order);
      onShowToast('تم إرسال إشعار التحويل للمراجعة ✅', 'success');
    } catch (error) {
      console.error(error);
      onShowToast(error instanceof Error ? error.message : 'تعذر رفع إشعار التحويل', 'error');
    } finally { setBusy(false); }
  };
  ...
  const adminSet = async (status: Order['status'], paid?: boolean) => {
    ...
    setBusy(true);
    try {
      const result = await updateFlaskOrderStatus(order.id, status, paid);
      if (result.order) onOrderChanged?.(result.order as Order);
      onShowToast(status === 'preparing' ? 'تم تأكيد الدفع وتجهيز الطلب ✅' : status === 'in_shipping' ? 'تم بدء الشحن 🚚' : 'تم تحديث حالة الطلب', 'success');
    } catch (error) {
      console.error(error);
      onShowToast(error instanceof Error ? error.message : 'تعذر تحديث الطلب', 'error');
    } finally { setBusy(false); }
  };
