...
export const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 text-slate-900 pb-20 sm:pb-8">
      ...
      <CartModal isOpen={isCartOpen} cartItems={cartItems} user={user} currency={currency} onClose={() => setIsCartOpen(false)} onUpdateQuantity={handleUpdateCartQuantity} onRemoveItem={handleRemoveCartItem} onClearCart={() => setCartItems([])} onOrderPlaced={(newOrder) => { setOrders((current) => [newOrder, ...current.filter((item) => item.id !== newOrder.id)]); setSelectedChatOrder(newOrder); }} onShowToast={showToast}/>
